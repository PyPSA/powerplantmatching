...
from powerplantmatching.collection import combine_multiple_datasets as cmd
...
from powerplantmatching.data import CARMA, ENTSOE, ESE, GEO, GPD, OPSD, WEPP, WRI
...
ese = pm.data.ESE()
geo = pm.data.GEO()
opsd = pm.data.OPSD().assign(Fueltype=lambda df: df.Fueltype.fillna("Hard Coal"))
...
for i, name in df_dict.items():
    dfp = locals()[str.lower(name)].loc[lambda x: ~x.Fueltype.isin(excluded_fueltypes)]
    dfp.loc[:, "matches"] = float(i) + 0.4 * (2.0 * np.random.rand(len(dfp)) - 1.0)
...
ax.legend(
    [
        plt.Line2D(
            [0, 0], [0, 0], color=fueltype_to_color[f], lw=0, markersize=8.0, marker="o"
        )
        for f in pd.Series(ft)
    ],
    ft,
    frameon=False,
)
...
opsd = (
    OPSD()
    .pipe(pm.cleaning.aggregate_units, dataset_name="OPSD", use_saved_aggregation=True)
...
# Plot
fig, ax = pm.plot.fueltype_totals_bar(
    dfs, keys, axes_style="darkgrid", figsize=(figwidth, 4)
)
ax.set_xlabel("")
ax.grid(color="white", linestyle="dotted")
...
fig, ax = pm.plot.comparison_1dim(
    include_WEPP=False,
    by="Fueltype",
    axes_style="darkgrid",
...
df["matches"] += 0.4 * (2.0 * np.random.rand(len(df)) - 1.0)
...
# Plot
fig, ax = plt.subplots(nrows=2, sharey=False, sharex=True, figsize=(figwidth, 7.0))
df.plot.scatter(
    x="Capacity",
    y="matches",
    logx=True,
...
ax[0].legend(
    [
        plt.Line2D([0, 0], [0, 0], color=c, lw=0, markersize=4.0, marker="o")
...
yr_wepp = df_w_wepp.pivot_table(values="DateIn", index="Country", aggfunc="count")
count_wepp = df_w_wepp.pivot_table(values="Name", index="Country", aggfunc="count")
ratio_wepp = yr_wepp.iloc[:, 0].div(count_wepp.iloc[:, 0])

yr_wo = df_wo_wepp.pivot_table(values="DateIn", index="Country", aggfunc="count")
count_wo = df_wo_wepp.pivot_table(values="Name", index="Country", aggfunc="count")
...
lu = pm.utils.lookup(
    [m_with_wepp, m_without_wepp, stats],
