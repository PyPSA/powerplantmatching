...
OPSD_links = pd.read_csv('opsd_links.csv', index_col='id',
                         encoding='utf-8').drop('Unnamed: 0', axis=1)
OPSD_links.rename(columns={'ID BNetzA': 'BNETZA',
                           'uba_match_name': 'UBA'}, inplace=True)
...
# different version...)
OPSD_links = OPSD_links[(OPSD_links.BNETZA.isin(
    bnetza.projectID)) & (OPSD_links.UBA.isin(uba.Name))]
...
OPSD_links_r = OPSD_links.reset_index(
    drop=False).set_index('UBA').BNETZA.sort_values()


UBA, groups_uba = pm.cleaning.clean_single(
    uba, aggregate_powerplant_units=True, return_aggregation_groups=True)
BNETZA, groups_bnetza = pm.cleaning.clean_single(bnetza, aggregate_powerplant_units=True,
                                                 return_aggregation_groups=True)

MATCHED = pm.collection.combine_multiple_datasets(
    [UBA, BNETZA], ['UBA', 'BNETZA'])
...
opsd_links = OPSD_links.reset_index()
opsd_links = (opsd_links.assign(groups_BNETZA=lambda df: df.BNETZA.map(groups_bnetza))
              .assign(groups_UBA=lambda df: df.UBA.map(groups_uba)))

opsd_linking_groups = pd.DataFrame({'BNETZA': opsd_links.BNETZA.map(
    groups_bnetza), 'UBA': opsd_links.UBA.map(groups_uba)})
opsd_linking_groups = opsd_linking_groups.drop_duplicates()

print(opsd_linking_groups.apply(lambda df: df.duplicated()).sum())
is_duplicated = opsd_linking_groups.apply(
    lambda df: df.duplicated(keep=False)).any(axis=1)
...
GROUPS_bnetza = groups_bnetza.reset_index().groupby('grouped').aggregate(lambda df: df.tolist()).astype(str
                                                                                                        ).reset_index().set_index('projectID').grouped
GROUPS_uba = groups_uba.reset_index().groupby('grouped').aggregate(lambda df: df.tolist()).astype(str
                                                                                                  ).reset_index().set_index('projectID').grouped
ppm_linking_groups = pd.DataFrame({'BNETZA': MATCHED.projectID.BNETZA.astype(str).map(GROUPS_bnetza),
