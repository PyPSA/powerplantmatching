...
* Replaced the Java-based DUKE matching engine with a Splink (Fellegi-Sunter) record-linkage backend (`powerplantmatching.linkage`, built on `splink` + DuckDB). Matching no longer requires a Java installation or the bundled DUKE binaries. The `parallel_duke_processes` config key is renamed to `parallel_processes`.
