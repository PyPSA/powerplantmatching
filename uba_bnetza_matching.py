...
UBA, groups_uba = pm.cleaning.clean_single(uba, aggregate_powerplant_units=True
...
opsd_linking_groups = pd.DataFrame({'BNETZA':opsd_links.BNETZA.map(groups_bnetza), 'UBA':opsd_links.UBA.map(groups_uba)})
opsd_linking_groups = opsd_linking_groups.drop_duplicates()

print opsd_linking_groups.apply(lambda df:df.duplicated()).sum()
is_duplicated = opsd_linking_groups.apply(lambda df:df.duplicated(keep=False)).any(axis=1)
